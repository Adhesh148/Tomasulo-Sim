...
class FMULSolve:
	...
	def __init__(self,opnd1,opnd2,filename='fmul_tb.v'):
		self.opnd1 = opnd1
		self.opnd2 = opnd2
	...
	def solve(self):
		...
		file = self.genFMULtb(ie1,ie2,self.filename)
		cmd = ["iverilog",self.filename]
		p = subprocess.Popen(cmd,cwd="verilog/FMUL/")
		p.wait()
		fo = open("output.txt","w")
		subprocess.call(["./a.out"],cwd="verilog/FMUL/",stdout=fo)
		fo.close()
	...
	def genFMULtb(self,opnd1,opnd2,filename = 'fmul_tb.v'):
		str = '''\
	`include "fpMult.v"
	...
	module top;
		...
		filename = "verilog/FMUL/" + filename
		fo = open(filename,"w")
		fo.write(str)
		fo.close()
...
#obj = FMULSolve(opnd1=2.5,opnd2=-1.35)
#r = obj.solve()
#print(r)